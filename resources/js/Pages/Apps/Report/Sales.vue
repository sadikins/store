<script setup>
    ...
    const filter = () => {
        ...
        //HTTP request
        Inertia.get(route('apps.sales.filter'), {
    ...
    <div class="container w-full mt-5">
        ...
        <div v-if="sales">
            ...
            <thead>
                ...
                <th scope="col" class="text-left py-4 border-b border-gray-700">Cashier</th>
                <th scope="col" class="text-left py-4 border-b border-gray-700">Customer</th>
            ...
            <tr v-for="sale in sales" :key="sale.id" class="text-gray-500">
                ...
                <td class="py-4 border-b border-gray-700">{{ sale.created_at }}</td>
                <td class="py-4 border-b border-gray-700">{{ sale.invoice }}</td>
                <td class="py-4 border-b border-gray-700">{{ sale.cashier.name }}</td>
                <td class="py-4 border-b border-gray-700">{{ sale.customer ? sale.customer.name : 'Umum' }}</td>
                <td class="py-4 border-b border-gray-700">Rp. {{ formatPrice(sale.grand_total) }}</td>
            </tr>
            <tr class="bg-gray-700">

                <td colspan="5" class="py-3 text-white text-xl font-semibold pl-3">TOTAL</td>
        ...
        <table class="sm:w-full mt-3">
            ...
            <tbody>
            <tr v-for="sale in sales_report" :key="sale.id" class="text-gray-500">
                <td class="py-4 border-b border-gray-700">#</td>
                <td class="py-4 border-b border-gray-700">{{ sale.created_at }}</td>
                <td class="py-4 border-b border-gray-700">{{ sale.invoice }}</td>
                <td class="py-4 border-b border-gray-700">{{ sale.cashier.name }}</td>
